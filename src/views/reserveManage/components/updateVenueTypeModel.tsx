...
const UpdateVenueTypeModal: React.FC<CreateVenueTypeModalProps> = (
  {
    editData,
    isVisible,
    onClose,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response: any = await api.updateReservationInfo({
        id: editData.id,
  ...
  useEffect(() => {
    if (editData) {
      ...
      setIsANoonAvailable(editData.timeslots?.afternoon?.available ?? false);
      setIsEveningAvailable(editData.timeslots?.evening?.available ?? false);

      form.setFieldsValue({
        venueTypeName: editData.venueName ?? undefined,
        applicableAllDates: editData.isApplicableAllFutureDates ?? undefined,

        is_morning_available: editData.timeslots?.morning?.available ?? undefined,
        morningTime: editData.timeslots?.morning ? [
          editData.timeslots.morning.start ?
            dayjs(editData.timeslots.morning.start, 'HH:mm') : undefined,
        ...
        lunch_time: editData.timeslots?.lunchtime ? [
          editData.timeslots.lunchtime.start ?
            dayjs(editData.timeslots.lunchtime.start, 'HH:mm') : undefined,
          editData.timeslots.lunchtime.end ?
            dayjs(editData.timeslots.lunchtime.end, 'HH:mm') : undefined,
        ] : undefined,
        lunchtime_number: editData.timeslots?.lunchtime?.number ?? undefined,

        is_afternoon_time_available: editData.timeslots?.afternoon?.available ?? undefined,
        afternoon_time: editData.timeslots?.afternoon ? [
          editData.timeslots.afternoon.start ?
            dayjs(editData.timeslots.afternoon.start, 'HH:mm') : undefined,
          editData.timeslots.afternoon.end ?
            dayjs(editData.timeslots.afternoon.end, 'HH:mm') : undefined,
        ] : undefined,
        afternoon_number: editData.timeslots?.afternoon?.number ?? undefined,

        is_evening_time_available: editData.timeslots?.evening?.available ?? undefined,
        evening_time: editData.timeslots?.evening ? [
          editData.timeslots.evening.start ?
            dayjs(editData.timeslots.evening.start, 'HH:mm') : undefined,
          editData.timeslots.evening.end ?
            dayjs(editData.timeslots.evening.end, 'HH:mm') : undefined,
        ] : undefined,
        evening_number: editData.timeslots?.evening?.number ?? undefined,

        dates: editData.availableDays ? editData.availableDays : undefined,
      });
    }
  }, [editData, form]);
...
export default UpdateVenueTypeModal;
